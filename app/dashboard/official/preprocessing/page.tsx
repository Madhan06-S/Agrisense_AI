...
const GlobeViewer = dynamic(() => import("@/components/maps/GlobeViewer"), {
  ssr: false,
  loading: () => (
    <div className="w-full h-[600px] flex items-center justify-center bg-slate-950/80 rounded-2xl border border-white/5">
      <span className="text-slate-400 text-xs animate-pulse">Loading 3D Globe...</span>
    </div>
  )
});
...
const FarmTerrain3D = dynamic(() => import("@/components/maps/FarmTerrain3D"), {
  ssr: false,
  loading: () => (
    <div className="w-full h-[500px] flex items-center justify-center bg-slate-950/80 rounded-2xl border border-white/5">
      <span className="text-slate-400 text-xs animate-pulse">Loading 3D Terrain Model...</span>
    </div>
  )
});
...
const Pipeline3D = dynamic(() => import("@/components/animations/Pipeline3D"), {
  ssr: false,
  loading: () => (
    <div className="w-full h-[450px] flex items-center justify-center bg-slate-950/80 rounded-2xl border border-white/5">
      <span className="text-slate-400 text-xs animate-pulse">Loading 3D Pipeline...</span>
    </div>
  )
});
...
const Damage3DViewer = dynamic(() => import("@/components/claims/Damage3DViewer"), {
  ssr: false,
  loading: () => (
    <div className="w-full h-[480px] flex items-center justify-center bg-slate-950/80 rounded-2xl border border-white/5">
      <span className="text-slate-400 text-xs animate-pulse">Loading 3D Damage Assessor...</span>
    </div>
  )
});
...
export default function PreprocessingDashboard() {
  ...
  return (
    <main className="min-h-screen bg-[#020617] text-slate-100 p-8 font-sans">
      
      {/* Dashboard Top Navigation Header */}
      <header className="mb-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-6 pb-6 border-b border-white/5">
        <div>
        ...
        <div className="flex gap-3">
          <button className="flex items-center gap-2 py-2 px-4 rounded-xl text-xs font-semibold bg-emerald-500 hover:bg-emerald-600 text-slate-950 transition-colors shadow-lg shadow-emerald-500/20">
            <Play className="w-3.5 h-3.5 fill-current" />
            Trigger Reconstruction
          </button>
          <button className="flex items-center gap-2 py-2 px-4 rounded-xl text-xs font-semibold bg-slate-800 hover:bg-slate-700 text-slate-200 border border-white/10 transition-colors">
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8 mb-8">
        ...
        <HoloCard title="Temporal Gap Filling Heatmap" flippable={true} backContent={
          <div className="flex flex-col justify-between h-full">
            <div>
              <h4 className="text-xs font-bold text-emerald-400 uppercase mb-2">Heatmap Info</h4>
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="grid grid-cols-7 gap-2.5 bg-slate-950/40 p-4 rounded-xl border border-white/5">
              {heatmapDays.map((day) => (
                <div 
                  key={day.day}
                  className={`h-7 w-7 rounded flex items-center justify-center text-[10px] font-bold cursor-help transition-all hover:scale-110 ${
                    day.type === "optical" ? "bg-emerald-500/80 text-slate-950 shadow-sm shadow-emerald-500/30" :
            ...
            <div className="grid grid-cols-2 gap-2.5 text-[10px] font-semibold tracking-wider">
              <div className="flex gap-2 items-center">
                ...
                <span className="text-slate-300">Optical Available</span>
              </div>
              <div className="flex gap-2 items-center">
                <div className="w-2.5 h-2.5 rounded bg-cyan-500" />
                <span className="text-slate-300">SAR S1 Only</span>
              </div>
              <div className="flex gap-2 items-center">
                <div className="w-2.5 h-2.5 rounded bg-yellow-500" />
                <span className="text-slate-300">Interpolated Data</span>
              </div>
              <div className="flex gap-2 items-center">
                <div className="w-2.5 h-2.5 rounded bg-red-500" />
                <span className="text-slate-300">Severe Gap</span>
              </div>
            </div>
            ...
            <div className="mt-4 flex flex-col gap-2.5 border-t border-white/5 pt-4">
              ...
              <div className="flex justify-between items-center text-xs">
                <span className="text-slate-400">Interpolation Ratio</span>
                <span className="font-bold text-yellow-400">14.2%</span>
              </div>
              <div className="flex justify-between items-center text-xs">
                <span className="text-slate-400">Unfillable Gaps</span>
